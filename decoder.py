...
class Decoder(object):
    ...
    def wer(self, s1, s2):
        """
        Computes the Word Error Rate, defined as thee edit distance between the 
        tweo provided sentences after tokenizing to words.
        Arguments:
        ...
        #map the words to a char array (Levenshteing packages only accepts strings)
        w1 = [char(word2char[w]) for w in s1.split()]
        w2 = [char(word2char[w]) for w in s2.split()]
    ...
    def convert_to_strings(self, out, seq_len):
        ...
        for b, batch in enumerate(out):
            ...
            for p, utt in enumerate(batch):
                size = seq_len[b][p]
                if size > 0:
                    transcipt = ''.join(map(lambda x: self.int_to_char[x.item()], utt[0:size]))
                else:
                    transcipt = ''
